...
#include <esp_wifi.h> //libreria para el manejo de canales WiFi
...
//WiFi
constexpr char WIFI_SSID[] = "iPhone de Juliana";
...
float leer_bat() {
  return (((analogRead(bat) * 3.3 / 4095) * 1.51)*100)/5;
}
...
void setup() {
  ...
  //se establece unn segundo canal para evitar interferencia entre ambos protocolos
...
void loop() {
  ...
  int act_laser = 0;
  float valorAleatorio = random(256);
  ...
  //ESP-NOW
  //se establecen los valores a enviar
  ...
  //confirmar si el envio fue exitoso
  if (result == ESP_OK) {
